...
namespace PSSDKPlotPeakDetectionExample
{
    public partial class Form1 : Form
    {
        ...
        private void btnMeasure_Click(object sender, EventArgs e)
        {
            if (psCommSimpleWinForms.DeviceState == PalmSens.Comm.CommManager.DeviceState.Idle) //Determine whether the device is currently idle or measuring
            {
                try
                {
                    ...
                    _activeMeasurement = psCommSimpleWinForms.Measure(_methodCV); //Start measurement defined in the method
                }
                catch (Exception ex)
                {
                    lbConsole.Items.Add(ex.Message);
        ...
        private void DetectLSVCVPeaks()
        {
            ...
            semiDerivativePeakDetection.GetNonOverlappingPeaks(curves, peakDetectProgress);
        }
